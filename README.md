...
> [my-neuro(肥牛)](https://github.com/morettt/my-neuro) 社区插件发布仓库 · 需要一个 QQ 小号 + LLBot 协议端
...
- 简单说：**给你的肥牛办一张 QQ 号**。装好后，你用手机 QQ 私聊她 = 直接和电脑前的她对话，聊的是**同一段记忆**（电脑上聊到一半，出门在 QQ 上接着聊，她都记得）
- 不止聊天：QQ 上还能远程让她**帮你查文件、跑命令、播报消息**——等于出门在外也带着一个能操作你电脑的助手（当然有白名单和安全限制）
- **需要两样东西**：① 一个 QQ 小号当她的"身体"（别用大号，容易被风控）；② 一个叫 LLBot 的程序负责把 QQ 消息翻译给插件（下面有从下载到登录的保姆级教程）
...
**方式一（推荐）：WebUI 插件广场一键安装**——WebUI 控制面板 → 「广场」→ 插件广场 → 找到本插件点安装（新机器记得先跑一次仓库里的 `安装LLBOT.bat` 下载 LLBot 协议端，见上文 1.1）。

**方式二：手动放置**——将整个仓库内容放到 my-neuro 的社区插件路径下：
...
- 用你的大号（已设为 admin）加小号好友，私聊发「在吗」→ 电脑前的肥牛会用同样的记忆和人格回复你
- 「帮我看看 D 盘工作目录里今天新加了什么文件」→ 远程调用文件工具
- 「把《xx》文档里第一段念出来」→ 桌面 TTS 播报
- 群里 @ 小号提问 → 独立会话回答（不污染电脑端对话流）
- 出门在外：「我快到家了，提前十分钟开空调」（配合其他插件工具）

## 更新与卸载

- **更新**：插件广场点更新，或手动覆盖 `community/qq-connect/` 后重启；配置文件不会被冲掉，LLBot 是独立程序不受影响
- **卸载**：WebUI「插件管理」停用并删除 `live-2d/plugins/community/qq-connect/` 文件夹即可；LLBot 与 QQ 小号登录态如不再使用，建议自行退出并卸载
